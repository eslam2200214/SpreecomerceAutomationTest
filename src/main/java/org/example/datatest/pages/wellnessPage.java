...
public class wellnessPage extends filter {
    private static final Logger log = LoggerFactory.getLogger(FashionPage.class);

    @FindBy(xpath ="//span[text()=\"Wellness\"]")
    private WebElement wellnessButton;

    @FindBy(xpath ="//a[@data-title=\"sportswear\"]")
    private WebElement sportswear;

    @FindBy(xpath = "(//div[@id='products']//div[contains(@class,'swiper-slide')]//*[self::h2 or self::h3 or self::p])[3]")
    ...
    public void wellnessButton(){
        WaitUtils.waitForClickability(wellnessButton);
        wellnessButton.click();
    }

    public void sportswear(){
        WaitUtils.waitForClickability(sportswear);
        sportswear.click();
    }
